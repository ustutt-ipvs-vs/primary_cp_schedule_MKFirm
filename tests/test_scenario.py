    ...
    def test_scenario_loading(self):
        ...
        for stream in scenario.streams:
            if stream.id == '0':
                ...
                self.assertEqual(stream.frame_size_B, 100)
                self.assertEqual(stream.max_delay_ns, 6000000)
            elif stream.id == '1':
                self.assertEqual(stream.source, '3')
                self.assertEqual(stream.destination, '0')
                self.assertEqual(stream.cycle_time_ns, 6000000)
                self.assertEqual(stream.frame_size_B, 200)
                self.assertEqual(stream.max_delay_ns, 6000000)
            elif stream.id == '2':
                self.assertEqual(stream.source, '0')
                self.assertEqual(stream.destination, '3')
                ...
                self.assertEqual(stream.frame_size_B, 100)
                self.assertEqual(stream.max_delay_ns, 4000000)
            elif stream.id == '3':
                self.assertEqual(stream.source, '3')
                self.assertEqual(stream.destination, '0')
                self.assertEqual(stream.cycle_time_ns, 6000000)
